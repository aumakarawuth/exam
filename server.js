...
app.get('/api/students', requireAdmin, (req, res) => {
  ...
  list = [...list].sort((a, b) => (/\.\s*\d+\s*\//.test(String(a.classRoom || '')) ? 0 : 1) - (/\.\s*\d+\s*\//.test(String(b.classRoom || '')) ? 0 : 1) || String(a.classRoom ?? '').localeCompare(String(b.classRoom ?? ''), 'th', { numeric: true }) || String(a.studentId ?? '').localeCompare(String(b.studentId ?? ''), 'th', { numeric: true }));
  res.json(list);
});
