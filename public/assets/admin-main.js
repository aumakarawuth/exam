...
async function refreshOperations(){
  ...
  try{
    ...
    <h3 style="margin:18px 0 10px;">ความถูกต้องของคะแนน</h3><div class="operations-count-grid"><div class="operations-card"><div class="label">ตรวจสอบผ่าน</div><div class="value operations-ok">${Number(scoreChecks.verified)||0}</div></div><div class="operations-card" role="button" tabindex="0" style="cursor:pointer;" onclick="openScoreVerificationIssues()" onkeydown="if(event.key==='Enter')openScoreVerificationIssues()"><div class="label">คะแนนไม่ตรง · กดดูรายละเอียด</div><div class="value ${scoreChecks.mismatch?'operations-warn':'operations-ok'}">${Number(scoreChecks.mismatch)||0}</div><div class="note">รายการนี้จะถูกบล็อกไม่ให้ประกาศ</div></div><div class="operations-card" role="button" tabindex="0" style="cursor:pointer;" onclick="openScoreVerificationIssues()" onkeydown="if(event.key==='Enter')openScoreVerificationIssues()"><div class="label">ตรวจสอบไม่ได้ · กดดูรายละเอียด</div><div class="value ${scoreChecks.unverifiable?'operations-warn':'operations-ok'}">${Number(scoreChecks.unverifiable)||0}</div></div></div>
