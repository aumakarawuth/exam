...
document.querySelectorAll('.admin-tab-btn').forEach(btn=>{
  btn.addEventListener('click', ()=>{
    ...
    if(btn.dataset.atab==='operations') refreshOperations();
...
async function refreshOperations(){
  ...
  try{
    const data=await apiGetOperations(),counts=data.counts||{},activity=data.recentActivity||[],requests=data.requests||{},submissions=data.submissions||{},scoreChecks=data.scoreVerification||{},examChecks=data.examReadiness||{},backup=data.backup||{},alerts=data.alerts||{},jobs=data.jobs||{};
    ...
    <h3 style="margin:18px 0 10px;">ความถูกต้องของคะแนน</h3><div class="operations-count-grid"><div class="operations-card"><div class="label">ตรวจสอบผ่าน</div><div class="value operations-ok">${Number(scoreChecks.verified)||0}</div></div><div class="operations-card"><div class="label">คะแนนไม่ตรง</div><div class="value ${scoreChecks.mismatch?'operations-warn':'operations-ok'}">${Number(scoreChecks.mismatch)||0}</div><div class="note">รายการนี้จะถูกบล็อกไม่ให้ประกาศ</div></div><div class="operations-card"><div class="label">ตรวจสอบไม่ได้</div><div class="value ${scoreChecks.unverifiable?'operations-warn':'operations-ok'}">${Number(scoreChecks.unverifiable)||0}</div></div></div>
