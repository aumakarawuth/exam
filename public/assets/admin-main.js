...
async function refreshOperations(){
  ...
  try{
    ...
    <h3 style="margin:18px 0 10px;">ความพร้อมของข้อสอบ</h3><div class="operations-count-grid"><div class="operations-card"><div class="label">พร้อมเปิดสอบ</div><div class="value operations-ok">${Number(examChecks.ready)||0}</div></div><div class="operations-card"><div class="label">ถูกบล็อกเพื่อรอแก้ไข</div><div class="value ${examChecks.blocked?'operations-warn':'operations-ok'}">${Number(examChecks.blocked)||0}</div><div class="note">ข้อสอบที่ไม่ผ่านจะไม่แสดงให้นักเรียนเห็น</div></div></div>
    <h3 style="margin:18px 0 10px;">ความถูกต้องของคะแนน</h3><div class="operations-count-grid"><div class="operations-card"><div class="label">ตรวจสอบผ่าน</div><div class="value operations-ok">${Number(scoreChecks.verified)||0}</div></div><button type="button" class="operations-card score-verification-detail-btn" style="cursor:pointer;text-align:left;font:inherit;color:inherit;"><div class="label">คะแนนไม่ตรง · กดดูรายละเอียด</div><div class="value ${scoreChecks.mismatch?'operations-warn':'operations-ok'}">${Number(scoreChecks.mismatch)||0}</div><div class="note">รายการนี้จะถูกบล็อกไม่ให้ประกาศ</div></button><button type="button" class="operations-card score-verification-detail-btn" style="cursor:pointer;text-align:left;font:inherit;color:inherit;"><div class="label">ตรวจสอบไม่ได้ · กดดูรายละเอียด</div><div class="value ${scoreChecks.unverifiable?'operations-warn':'operations-ok'}">${Number(scoreChecks.unverifiable)||0}</div></button></div>
...
document.getElementById('operationsWrap').addEventListener('click',event=>{if(event.target.closest('.score-verification-detail-btn'))openScoreVerificationIssues();});
...
function renderResultGroups(records){
  ...
  return [...groups.values()].map(group=>`<div class="course-group result-subject-group">
    ...
    <div class="course-group-body collapsed" style="overflow-x:auto;"><table class="result-table"><thead><tr>
      ...
      <td><button class="pub-pill ${r.published?'pub-yes':'pub-no'}" data-togglepub="${r.id}">${r.published?'ประกาศแล้ว ✅':'ยังไม่ประกาศ 🔒'}</button></td><td><button class="btn btn-ghost btn-sm" data-viewdetail="${r.id}">ดูรายละเอียด</button><button class="btn btn-danger btn-sm" data-delres="${r.id}">ลบ</button></td>
