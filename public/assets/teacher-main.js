...
function initRosterTab(){
  ...
  try{
    ...
    if(!document.getElementById('rosterSchoolInput').value) document.getElementById('rosterSchoolInput').value=saved.school||'วิทยาลัยเทคโนโลยีจรัลสนิทวงศ์';
    if(!document.getElementById('rosterAddressInput').value) document.getElementById('rosterAddressInput').value=saved.address||'18 ซอย จรัญสนิทวงศ์ 41 แขวงอรุณอมรินทร์ เขตบางกอกน้อย กรุงเทพมหานคร 10700';
    if(!document.getElementById('rosterProgramInput').value) document.getElementById('rosterProgramInput').value=saved.program||'';
  }catch(error){}
}
...
document.getElementById('printRosterBtn').addEventListener('click',async()=>{
  ...
  const options={school:document.getElementById('rosterSchoolInput').value.trim(),address:document.getElementById('rosterAddressInput').value.trim(),program:document.getElementById('rosterProgramInput').value.trim(),examRoom:document.getElementById('rosterExamRoomInput').value.trim(),examLink:document.getElementById('rosterExamLinkInput').value.trim()};
  try{localStorage.setItem(ROSTER_HEADER_STORAGE_KEY,JSON.stringify({school:options.school,address:options.address,program:options.program}));}catch(error){}
