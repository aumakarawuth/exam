...
function registerAdminSetRoutes(app, { readDB, writeDB, requireAdmin, examTypes, newId, applyAcademicPeriod }) {
  ...
  app.post('/api/sets', requireAdmin, async (req, res) => {
    ...
    const set = { key, title: body.title, courseName: body.courseName || body.title, educationLevel: body.educationLevel, tagline: body.tagline || '', desc: body.desc || '', examType: examTypes.includes(body.examType) ? body.examType : examTypes[0], teacherId: body.teacherId || null, assignedClasses: Array.isArray(body.assignedClasses) ? body.assignedClasses : [], examSchedules: normalizeSchedules(body.examSchedules), subjectTeacherName: body.subjectTeacherName || '', subjectTeacherEmail: body.subjectTeacherEmail || '', shuffleQuestions: !!body.shuffleQuestions, shuffleChoices: !!body.shuffleChoices, publishMode: body.publishMode === 'auto' ? 'auto' : 'manual', delivery: body.delivery === 'object-analysis-design' ? 'object-analysis-design' : null, availableFrom: normalizeExamDateTime(body.availableFrom) || null, availableUntil: normalizeExamDateTime(body.availableUntil) || null, lateAccessCode: body.lateAccessCode || '', sections: body.sections, createdAt: now, updatedAt: now };
  ...
  app.put('/api/sets/:key', requireAdmin, async (req, res) => {
    ...
    if (errors.length) return sendValidationError(res, errors);
    db.sets[index] = Object.assign({}, old, { title: body.title, courseName: body.courseName || body.title, educationLevel: body.educationLevel, tagline: body.tagline || '', desc: body.desc || '', examType: examTypes.includes(body.examType) ? body.examType : (old.examType || examTypes[0]), teacherId: body.teacherId !== undefined ? body.teacherId : old.teacherId, assignedClasses: Array.isArray(body.assignedClasses) ? body.assignedClasses : [], examSchedules: normalizeSchedules(body.examSchedules), subjectTeacherName: body.subjectTeacherName || '', subjectTeacherEmail: body.subjectTeacherEmail || '', shuffleQuestions: !!body.shuffleQuestions, shuffleChoices: !!body.shuffleChoices, publishMode: body.publishMode === 'auto' ? 'auto' : 'manual', availableFrom: normalizeExamDateTime(body.availableFrom) || null, availableUntil: normalizeExamDateTime(body.availableUntil) || null, lateAccessCode: body.lateAccessCode || '', sections: body.sections, updatedAt: new Date().toISOString() });
