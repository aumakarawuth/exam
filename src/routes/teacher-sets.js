function registerTeacherSetRoutes(app, { readDB, writeDB, requireTeacher, examTypes, newId }) {
  ...
  app.post('/api/teacher/sets', requireTeacher, async (req, res) => {
    ...
    db.sets.push({ ...body, key, teacherId: req.teacherId, examType: examTypes.includes(body.examType) ? body.examType : examTypes[0], assignedClasses: Array.isArray(body.assignedClasses) ? body.assignedClasses : [], publishMode: body.publishMode === 'auto' ? 'auto' : 'manual', createdAt: now, updatedAt: now }); await writeDB(db); res.status(201).json({ key });
  });
  app.put('/api/teacher/sets/:key', requireTeacher, async (req, res) => {
    ...
    Object.assign(set, { ...req.body, key: set.key, teacherId: set.teacherId, examType: examTypes.includes(req.body.examType) ? req.body.examType : set.examType, assignedClasses: Array.isArray(req.body.assignedClasses) ? req.body.assignedClasses : [], updatedAt: new Date().toISOString() }); await writeDB(db); res.json({ ok: true });
  });
